...
//or stack is empty;
		if(YES == Is_empty(stack_order_procs) || YES == flag_re_fec)
		{
			if(1 == flag_re_fec)
			{
				compute_amount_pro_fec(RE_FEC, num_block, &limit_amount);
				while(limit_amount--) 	
				{
			...
			{
				compute_amount_pro_fec(PRO_FEC, num_block, &limit_amount); 
       		     while(limit_amount--)
       	    	{
                push(&stack_order_procs, num_block, amount_limit);
            	}
		...
		if(!transimit_over_ch(num_rand, packet_loss_rate)) // this packet will be erased if num_rand < packet_loss_rate
		{
//			sent_total_amount++;
